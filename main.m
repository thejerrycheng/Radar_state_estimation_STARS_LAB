...
std_dev_prior = [meas_sigma^2, meas_sigma^2, 3, (30/180*pi)^2, (30/180*pi)^2];
std_dev_mm = [0.1, 0.1, 0.1];
...
x = [reshape([noisy_poses(1:2, 1:window_length); data.rot_vel_vi(1:window_length) * norm(t_A_BA)], [], 1);...
...
for j = 1:length(data.t) - window_length
    ...
    for i = 1:20
        [prior_err, J_prior, P] = batch_prior(init_state, x, std_dev_prior);
        [motion_err, F, Q] = batch_f(x, std_dev_mm);
        [meas_err, G, R_B] = batch_g(current_meas_window, x);
        H = [J_prior; F; G];
    ...
    P_temp = diag(inv(A));
    P = diag([P_temp(1:3, 1); P_temp(end-1:end, 1)]);
...
for i=1:4
    switch i
        ...
        case 2
            ...
            hold on
            title(title_names(i))
            plot(0, x_store(3, 1)- data.rot_vel_vi(1) * norm(t_A_BA), 'ok')
